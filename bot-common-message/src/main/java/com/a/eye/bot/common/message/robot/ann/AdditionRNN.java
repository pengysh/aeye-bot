...
public class AdditionRNN {
    ...
    public static final int FEATURE_VEC_SIZE = 30;
    ...
    public static void main(String[] args) throws Exception {
        ...
        QACollection collection = new QACollection();
        collection.init();

        CustomSequenceIterator iterator = new CustomSequenceIterator(seed, batchSize, totalBatches, NUM_DIGITS, timeSteps, collection);
        ...
        while (iEpoch < nEpochs) {
            ...
            char[][] testNums = iterator.testFeatures();
            char[][] testSums = iterator.testLabels();
            ...
            encode_decode(testNums, testSums, answers);
    ...
    private static void encode_decode(char[][] num1, char[][] sum, INDArray answers) {
        ...
        for (int iTest = 0; iTest < nTests; iTest++) {
            ...
            while (aDigit >= 0) {
                ...
                double[] outputProbDistribution = new double[1];
                for (int k = 0; k < outputProbDistribution.length; k++) {
                    outputProbDistribution[k] = answers.getDouble(k);
                }
                System.out.print(gson.toJson(outputProbDistribution) + "\t");
                int sampledCharacterIdx = findIndexOfHighestValue(outputProbDistribution);
                System.out.println(sampledCharacterIdx);
            ...
            if (strAnswerR.equals(String.valueOf(sum[iTest]))) {
                System.out.println(gson.toJson(num1[iTest]) + "==" + gson.toJson(strAnswerR));
                correct++;
            } else {
                System.out.println(gson.toJson(num1[iTest]) + "!=" + strAnswerR + ", should==" + gson.toJson(sum[iTest]));
                wrong++;
            }
        }
